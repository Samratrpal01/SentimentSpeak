...
about_label = Label(tab3,text="NLPiffy GUI V.0.0.1 \n Final Year Project \n@Rahul Pal, @Akash Saraswat \n@Nikhil Gupta and @Nitikesh Singh",pady=5,padx=5)
...
def get_sentiment():
	...
	if new_text.sentiment.polarity>0.2:
    		temp="Positive"
	elif new_text.sentiment.polarity==0.0 and new_text.sentiment.subjectivity==0.0:
		temp="Normal"
	elif new_text.sentiment.polarity<=0.2:
		temp="Negative"
	result = '\nSubjectivity:{}, Polarity:{}, Remarks:{}'.format(new_text.sentiment.subjectivity,new_text.sentiment.polarity,temp)
	tab1_display.insert(tk.END,result)
...
def listen_voice():
     ...
     try:
          displayed_file.insert(tk.END,r.recognize_google(audio))
...
# bUTTONS
button1=Button(tab1,text="Tokenize", width=12,command=get_tokens,bg='#03A9F4')
...
button3=Button(tab1,text="Sentiment", width=12,command=get_sentiment,bg='#f44336')
...
displayed_file.grid(row=2,column=0, columnspan=4,padx=5,pady=5)


# BUTTONS FOR SECOND TAB/FILE READING TAB
b0=Button(tab2,text="Open File", width=11,command=openfiles,bg='#c5cae9')
b0.grid(row=3,column=0,padx=10,pady=10)

b1=Button(tab2,text="Reset ", width=11,command=clear_text_file,bg="#b9f6ca")
b1.grid(row=3,column=1,padx=10,pady=10)

b1a=Button(tab2,text="NLpiffy", width=11,command=nlpiffy_file,bg='blue')
b1a.grid(row=3,column=2,padx=10,pady=10)

b2=Button(tab2,text="Tokenize", width=11,command=get_file_tokens,bg='#03A9F4')
b2.grid(row=4,column=0,padx=10,pady=10)


b3=Button(tab2,text="POS Tags", width=11,command=get_file_pos_tags,bg='#BB86FC')
b3.grid(row=4,column=1,padx=10,pady=10)


b4=Button(tab2,text="Sentiment", width=11,command=get_file_sentiment,bg='#f44336')
b4.grid(row=4,column=2,padx=10,pady=10)


b5=Button(tab2,text="Entities", width=11,command=get_file_entities,bg='#80d8ff')
b5.grid(row=5,column=0,padx=10,pady=10)


b6=Button(tab2,text="Clear Result", width=11,command=clear_result)
b6.grid(row=5,column=1,padx=10,pady=10)

b7=Button(tab2,text="Close", width=11,command=window.destroy)
b7.grid(row=5,column=2,padx=10,pady=10)

b8=Button(tab2,text="Speak", width=36,command=listen_voice)
b8.grid(row=6,column=1,padx=10,pady=10)
